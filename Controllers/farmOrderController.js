...
exports.placeOrder = catchAsync(async (req, res, next) => {
  try {
    ...
    const buyerOrders = [...buyer.farmOrders, doc.id];

    await Buyer.findByIdAndUpdate(req.body.buyer, {
      farmOrders: buyerOrders,
      cart: [],
      cartQty: [],
    });
    ...
    products.forEach(async (el, i) => {
      ...
      const sellerOrders = [...seller.farmOrders, doc.id];

      await Seller.findByIdAndUpdate(el.seller.id, {
        farmOrders: sellerOrders,
      });
    });
...
exports.deleteOrder = catchAsync(async (req, res, next) => {
  ...
  let buyerOrders = [...buyer.farmOrders];
  ...
  products.forEach(async (el, i) => {
    ...
    const seller = await Seller.findById(el.seller.id);
    let sellerOrders = [...seller.farmOrders];
